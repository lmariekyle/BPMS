<x-app-layout>
    ...
    <div class="w-[1400px] h-[813px] ml-8 mt-16 px-5 py-5 rounded-lg border-2" style="border-color: black;">
        ...
        <div class="justify-center flex flex-row mt-14">
            <a href="{{ route('approve', $transaction->id) }}" class="text-center w-[400px] font-robotocondensed font-bold text-[32px] text-dirty-white bg-deep-green px-4 py-2" style="width: 400px; font-size: 32px;"><button type="submit" <?php if($transaction->payment['paymentMethod'] == 1 || $transaction->payment['paymentStatus'] != 'Paid') {?> disabled <?php } ?>>Approve Request </button></a>
            <a href="{{ route('deny', $transaction->id) }}" class="text-center w-[400px] ml-8 font-robotocondensed font-bold text-[32px] text-dirty-white px-4 py-2" style="width: 400px; font-size: 32px; background-color: #D86F4D;">Deny Request</a>
        </div>
    </div>
